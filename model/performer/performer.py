...
class SelfAttentionNotWorking(nn.Module):
    def __init__(
        self,
        dim,
        causal = False,
    ...
    def forward(self, x, pos_emb=None, context=None, mask=None, context_mask=None, output_attentions=False, **kwargs):
        ...
        q, k, v = self.to_q(context), self.to_k(context), self.to_v(context)
        
        # qkv = torch.stack([q, k, v], dim=-2)

        # qkv = self.to_qkv(context)
        # qkv = rearrange(qkv, "b n (h d three) -> b h n three d", h=h, three=3)
        # q, k, v = qkv.unbind(dim=-2)

        if self.use_flash_attention:
            qkv = torch.stack([q, k, v], dim=-2)
            out = flash_attn_qkvpacked_func(q, k, v, causal=self.causal)
        else:
            (q, lq), (k, lk), (v, lv) = map(lambda t: (t[:, :gh], t[:, gh:]), (q, k, v))
            attn_outs = []
